...
exports.createSiren = async (req, res) => {
  ...
  try {
    ...
    // 1) Récupération Sirene
    const entreprises = await fetchEntreprisesForDepartement(dep);

    let inserted = 0;
    let updated = 0;

    // enrichments seed (utile pour éviter de repayer)
    let seededPappers = 0;
    let seededDropEntreprise = 0;

    await conn.beginTransaction();

    for (const p of entreprises) {
      // Mapping attendu depuis formatEtab()
      const siren = p.siren;
      const denomination = p.denomination;
      const adresse = p.adresse;
      const ville = p.ville;
      const activite_code = p.activite_code;
      const tranche_effectif = p.tranche_effectif;
      ...
      const [result] = await conn.query(
        ...
        `,
        [siren, denomination, adresse, ville, activite_code, tranche_effectif, dep]
      );

      // MySQL: INSERT => affectedRows=1, UPDATE => affectedRows=2 (si modif)
      if (result.affectedRows === 1) inserted++;
      if (result.affectedRows === 2) updated++;

      // 3) Récupérer l'id entreprise (nouvelle ou existante)
      let entrepriseId = result.insertId;
      if (!entrepriseId) {
        const [rows] = await conn.query(
          `SELECT id FROM entreprises WHERE siren = ? LIMIT 1`,
          [siren]
        );
      ...
      // 4) Seed enrichments intelligemment (éviter repayer)
      // 4a) PAPPERS: on ne seed que si on n'a pas déjà des dirigeants
      const [[{ cntDir }]] = await conn.query(
        `SELECT COUNT(*) AS cntDir FROM dirigeants WHERE entreprise_id = ?`,
        [entrepriseId]
      );

      if (Number(cntDir) === 0) {
        const [seedP] = await conn.query(
          `
          INSERT IGNORE INTO enrichments (entity_type, entity_id, provider, status)
          VALUES ('ENTREPRISE', ?, 'PAPPERS', 'NEVER')
          `,
          [entrepriseId]
        );
        seededPappers += seedP.affectedRows || 0;
      }

      // 4b) DROPCONTACT (ENTREPRISE): on seed seulement si email NULL
      const [[rowEmail]] = await conn.query(
        `SELECT email FROM entreprises WHERE id = ? LIMIT 1`,
        [entrepriseId]
      );

      if (!rowEmail?.email) {
        const [seedD] = await conn.query(
          `
          INSERT IGNORE INTO enrichments (entity_type, entity_id, provider, status)
          VALUES ('ENTREPRISE', ?, 'DROPCONTACT', 'NEVER')
          `,
          [entrepriseId]
        );
        seededDropEntreprise += seedD.affectedRows || 0;
      }
    }
    ...
    return res.status(201).json({
      ...
      updated,
      seeded: {
        pappers: seededPappers,
        dropcontactEntreprise: seededDropEntreprise,
      },
    });
  } catch (err) {
    await conn.rollback();
